<div id="defaultModal{{$data->id_pemesanan}}" tabindex="-1" aria-hidden="true" class="fixed top-0 left-0 right-0 z-50 hidden w-full p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-[calc(100%-1rem)] max-h-full">
                ...
                <form action="/upload-transaksi/{{$data->id_pemesanan}}" method="post" enctype="multipart/form-data">
                    ...
                    <div class="mb-4">
                        <label for="bukti_tf" class="block text-gray-700 font-bold text-lg mb-2">Foto Bukti TF
                            (PNG/JPG)</label>
                        <input type="file" name="bukti_tf" id="bukti_tf" class="appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline text-lg" accept=".png, .jpg, .jpeg" required>
                    </div>
